...
class Item
{
    ...
    protected $target = 'self';   // Type of URL to generate URL, Route, Action
    ...
    public function __construct($text = '', array $properties = [])
    {
        ...
        if (strtolower($this->method) != 'get') {
            $this->form = sprintf(
                '<form id="%1$s" action="%2$s" method="POST" style="display: none;">%3$s</form>',
                $this->uniqid,
    ...
    public function __toString()
    {
        ...
        try {
            ...
            $result = sprintf(
                '<a%1$s href="%2$s"%3$s%4$s>%5$s</a>%6$s',
                " class=\"{$this->getClasses()}\" target=\"_{$this->target}\"",
                $this->getLocation(),
